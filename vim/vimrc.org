...
" {{{ || highlight || ---
highlight PmenuSel ctermfg=254 ctermbg=240 gui=bold guibg=Blue
highlight Pmenu ctermfg=254 ctermbg=236 guibg=DarkGrey
...
require('gitsigns').setup{
  on_attach = function(bufnr)
    ...
    -- Actions
    map({'n', 'v'}, '<leader>vhs', ':Gitsigns stage_hunk<CR>')
    map({'n', 'v'}, '<leader>vhr', ':Gitsigns reset_hunk<CR>')
...
let g:indentLine_fileTypeExclude = ['markdown', 'vimwiki', 'json', 'nerdtree', 'NvimTree', 'man']
...
EOF
function! s:InitNvimTreeCmds()
  let l:nvim_tree_cmds = {
        \ 'n': runcmds#init#MakeCmdInfo('NvimTreeOpen', v:false, [], v:true),
        \ 'N': runcmds#init#MakeCmdInfo('NvimTreeOpen %:h'),
        \ 't': runcmds#init#MakeCmdInfo('NvimTreeToggle'),
        \ 'o': runcmds#init#MakeCmdInfo('NvimTreeFocus'),
        \ 'f': runcmds#init#MakeCmdInfo('NvimTreeFindFile'),
        \ 'r': runcmds#init#MakeCmdInfo('NvimTreeRefresh'),
        \ }
  function! s:NvimTreeCmds() closure
    return l:nvim_tree_cmds
  endfunction
endfunction
function! s:InitNvimTreeFlagDict()
  let l:nvim_tree_flags = runcmds#init#MakeFlagDict(';', "\<Space>")
  function! s:NvimTreeFlags() closure
    return l:nvim_tree_flags
  endfunction
endfunction
call s:InitNvimTreeCmds()
call s:InitNvimTreeFlagDict()
nnoremap <expr> [Chief]t runcmds#base#RunCmds("NvimTree", <SID>NvimTreeCmds(), <SID>NvimTreeFlags())
" --- || nvim-tree.lua || }}}
...
for _, mode in ipairs({'n', 'v', 'o'}) do
  vim.api.nvim_set_keymap(mode, 'f', "<Cmd>HopChar1CurrentLineAC<CR>", {})
  vim.api.nvim_set_keymap(mode, 'F', "<Cmd>HopChar1CurrentLineBC<CR>", {})
  vim.api.nvim_set_keymap(mode, 'sw', "<Cmd>HopWordAC<CR>", {})
  vim.api.nvim_set_keymap(mode, 'sb', "<Cmd>HopWordBC<CR>", {})
  vim.api.nvim_set_keymap(mode, 'sj', "<Cmd>HopLineAC<CR>", {})
  vim.api.nvim_set_keymap(mode, 'sk', "<Cmd>HopLineBC<CR>", {})
  vim.api.nvim_set_keymap(mode, 's/', "<Cmd>HopPatternAC<CR>", {})
  vim.api.nvim_set_keymap(mode, 's?', "<Cmd>HopPatternBC<CR>", {})
end
...
"           \ 'r': runcmds#init#MakeCmdInfo(g:fzf_command_prefix . 'Rgh', v:false, [], v:true),
...
"           \ 'q': runcmds#init#MakeCmdInfo(g:fzf_command_prefix . 'Ghq'),
"           \ }
...
" command! -bang -nargs=* -complete=dir FzfRgh call fzf#vim#grep("rg --column --line-number --no-heading --color=always --smart-case --hidden -- ".shellescape(<q-args>), 1, fzf#vim#with_preview(), <bang>0)
" command! -bang -nargs=* FzfGhq call fzf#run(fzf#wrap({'source': 'ghq list --full-path', 'sink': 'NERDTree'}))
" endif
" --- || fzf.vim || }}}
...
function! s:InitFzfCmds()
  let l:fzf_cmds = {
        \ 'f': runcmds#init#MakeCmdInfo('Telescope find_files search_dirs=.'),
        \ 'r': runcmds#init#MakeCmdInfo('Telescope live_grep search_dirs=.'),
        \ 'b': runcmds#init#MakeCmdInfo('Telescope buffers'),
        \ 'o': runcmds#init#MakeCmdInfo('Telescope oldfiles'),
        \ 'l': runcmds#init#MakeCmdInfo('Telescope current_buffer_fuzzy_find'),
        \ 'g': runcmds#init#MakeCmdInfo('Telescope git_files'),
        \ 'G': runcmds#init#MakeCmdInfo('Telescope git_status'),
        \ 'C': runcmds#init#MakeCmdInfo('Telescope git_commits'),
        \ 'c': runcmds#init#MakeCmdInfo('Telescope git_bcommits'),
        \ 'h': runcmds#init#MakeCmdInfo('Telescope help_tags'),
        \ 'e': runcmds#init#MakeCmdInfo('Telescope diagnostics'),
        \ ':': runcmds#init#MakeCmdInfo('Telescope command_history'),
        \ '/': runcmds#init#MakeCmdInfo('Telescope search_history'),
        \ 'm': runcmds#init#MakeCmdInfo('Telescope marks'),
        \ 'k': runcmds#init#MakeCmdInfo('Telescope keymaps'),
        \ 's': runcmds#init#MakeCmdInfo('Telescope spell_suggest'),
        \ 'H': runcmds#init#MakeCmdInfo('Telescope highlights'),
        \ 'q': runcmds#init#MakeCmdInfo('Telescope ghq list'),
        \ }
  function! s:FzfCmds() closure
    return l:fzf_cmds
  endfunction
...
lua << EOF
  ...
  vim.api.nvim_set_keymap('i', '<C-g><C-h>', '<cmd>lua vim.lsp.buf.signature_help()<CR>', opts)
  local wk = require("which-key")
  wk.register({
    ["<C-]>"] = { "<Cmd>lua vim.lsp.buf.definition()<CR>", "Definition" },
    ["g<C-]>"] = { "<Cmd>lua vim.lsp.buf.implementation<CR>", "Implementation" },
    ["<C-w>"] = {
      ["<C-]>"] = { "<Cmd>split | lua vim.lsp.buf.definition()<CR>", "Definition [horz]" },
      ["<C-g><C-]>"] = { "<Cmd>split | lua vim.lsp.buf.implementation<CR>", "Implementation [horz]" },
    },
    ["[Vert]"] = {
      ["<C-]>"] = { "<Cmd>vertical split | lua vim.lsp.buf.definition()<CR>", "Definition [vert]" },
      ["<C-g><C-]>"] = { "<Cmd>vertical split | lua vim.lsp.buf.implementation<CR>", "Implementation [vert]" },
    },
    ["<C-t>"] = {
      ["<C-]>"] = { "<Cmd>tab split | lua vim.lsp.buf.definition()<CR>", "Definition [tab]" },
      ["<C-g><C-]>"] = { "<Cmd>tab split | lua vim.lsp.buf.implementation<CR>", "Implementation [tab]" },
    },
  })
...
lua << EOF
  ...
  for _, mode in ipairs({"i", "s"}) do
    vim.api.nvim_set_keymap(mode, "<C-]>", "<Plug>luasnip-expand-or-jump", {})
    vim.api.nvim_set_keymap(mode, "<C-s>", "<Plug>luasnip-jump-next", {})
    vim.api.nvim_set_keymap(mode, "<M-s>", "<Plug>luasnip-jump-prev", {})
  end
  vim.api.nvim_set_keymap("s", "<Tab>", "<Esc>i", { noremap = true })
  vim.api.nvim_set_keymap("s", "<C-a>", "<Esc>a", { noremap = true })
